...
def check_payment_status(invoice_number):
    try:
        ...
        if not payment_intents.data:
            return "No payment or pre-authorization found with this invoice number."
        ...
        status = status_messages.get(
            payment_intent.status, f"Unknown status: {payment_intent.status}")

        if payment_intent.metadata.get('is_preauth') == 'true':
            expiration_date = datetime.fromtimestamp(
                payment_intent.created) + timedelta(days=7)
            if datetime.now() > expiration_date:
                status += " (Pre-authorization expired)"
            else:
                status += f" (Pre-authorization expires on {expiration_date.strftime('%Y-%m-%d %H:%M:%S')})"
...
def main():
    ...
    with tab1:
        ...
        if amount > 0:
            ...
            st.write(
                f"Total amount (including fee): {amount + estimated_fee} {currency}")
            st.write(f"Estimated clearance time: {clearance_time}")

        payment_method = st.radio(
            "Select Payment Method", ["Credit/Debit Card", "Bank Transfer"])
        ...
        if payment_method == "Credit/Debit Card":
            ...
            if currency in BANK_TRANSFER_REQUIREMENTS:
                ...
                st.warning(
                    f"Bank transfer details for {currency} are not available. Please contact support for assistance.")
            stripe_payment_method = "customer_balance"

        if st.button("Confirm Payment"):
            payment_intent = create_payment_intent(
                amount, currency, stripe_payment_method, invoice_number, description)

            if payment_intent:
                st.success("Payment Intent created successfully!")
                st.json(payment_intent)
                st.info(
                    f"Use this Client Secret to complete the payment: {payment_intent.client_secret}")
                st.info(
                    f"Your invoice number is: {invoice_number}. Please save this for tracking your payment.")

                if payment_method == "Bank Transfer":
                    st.warning(
                        "For bank transfers, please use the payment instructions provided by our support team to complete the transaction.")
            ...
            st.write(
                "This payment uses Adaptive Pricing, which automatically selects the best payment method based on the customer's location and preferences.")
    ...
    with tab2:
        ...
        if st.button("Track Payment/Pre-authorization"):
            if tracking_invoice_number:
                ...
                st.warning(
                    "Please enter an invoice number to track your payment or pre-authorization.")

    with tab3:
        st.header("Pre-authorization")
        st.write(
            "Pre-authorize a payment amount without capturing it immediately.")

        st.warning(
            "⚠️ Important: The pre-authorization will expire in 7 days if not captured. After expiration, the funds will be released back to the card holder.")

        preauth_amount = st.number_input(
            "Pre-authorization Amount", min_value=0.01, step=0.01, value=10.00)
        ...
        preauth_card_number = st.text_input(
            "Card Number", key="preauth_card_number")
        preauth_exp_month = st.text_input(
            "Expiration Month (MM)", key="preauth_exp_month")
        preauth_exp_year = st.text_input(
            "Expiration Year (YYYY)", key="preauth_exp_year")
        ...
        if st.button("Pre-authorize Payment"):
            ...
            preauth_intent = create_payment_intent(
                preauth_amount, preauth_currency, "card", preauth_invoice_number, preauth_description, capture=False, is_preauth=True)

            if preauth_intent:
                st.success("Pre-authorization successful!")
                st.json(preauth_intent)
                st.info(
                    f"Pre-authorization Invoice Number: {preauth_invoice_number}")

                st.write(f"Pre-authorization Status: {preauth_intent.status}")
                expiration_date = datetime.fromtimestamp(
                    preauth_intent.created) + timedelta(days=7)
                st.write(
                    f"Pre-authorization Expiration Date: {expiration_date.strftime('%Y-%m-%d %H:%M:%S')}")

                st.warning(
                    "Remember: This pre-authorization will expire in 7 days if not captured. After expiration, the funds will be released.")
                st.info(
                    f"You can track the status of your pre-authorization using the invoice number: {preauth_invoice_number}")

    with tab4:
        st.header("Bank Account Details")
        st.warning(
            "This is for direct bank deposit only. You will need to pay directly from your own bank.")

        selected_currency = st.selectbox(
            "Select Currency", list(BANK_ACCOUNT_DETAILS.keys()))
